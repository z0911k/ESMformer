...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.q = nn.Conv1d(dim, dim, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        B, N, C = x.shape
        q = self.q(x.permute(0, 2, 1)).permute(0, 2, 1).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        x = self.proj(q.transpose(1, 2).reshape(B, N, C) - x)
        x = self.proj_drop(x)
        return x
...
## TODO: Single-view multi-level feature extractor
...
class SMFE(nn.Module):
    def __init__(self, args):
        ...
        self.Transformer_encoder_1 = Transformer(int(args.smfe[0]), args.frames, args.frames * 2,
                                                 length=2 * args.n_joints, h=9)
        self.Transformer_encoder_2 = Transformer(int(args.smfe[1]), args.frames, args.frames * 2,
                                                 length=2 * args.n_joints, h=9)
        self.Transformer_encoder_3 = Transformer(int(args.smfe[2]), args.frames, args.frames * 2,
                                                 length=2 * args.n_joints, h=9)
